      ...
      const paidCategoryFilter = document.getElementById('paidCategoryFilter');
        ...
        async function loadData() {
            try{
               ...
               if (!freeResponse.ok) {
                  console.error(`Free Starters HTTP error! status: ${freeResponse.status}, URL: ${freeResponse.url}`);
                   throw new Error(`HTTP error! status: ${freeResponse.status}  Free Starters Failed `);
               }

               if (!paidResponse.ok) {
                  console.error(`Paid Starters HTTP error! status: ${paidResponse.status}, URL: ${paidResponse.url}`);
                  throw new Error(`HTTP error! status: ${paidResponse.status} Paid Starters Failed`);
                }


                console.log("Data fetched successfully, processing...");
               const freeData = await freeResponse.json();
                allFreeData.all = Object.values(freeData).flat(); //combine all the properties from the object into a single array.

                allPaidData = await paidResponse.json();
                console.log("All Free Data:", allFreeData);
                 console.log("All Paid Data:", allPaidData);
               filteredFreeData.all = allFreeData.all;
              filteredPaidData.all = allPaidData.all;
        ...
        function filterAndPaginate() {
           ...
           const selectedCategory = categoryFilter.value;
            ...
            let filteredItems = filteredFreeData.all;
          ...
          if(selectedCategory !== ''){
              ...
              console.log("Filtered after category:", filteredItems);
            }
        ...
        function filterAndPaginatePaid() {
          ...
          const selectedCategory = paidCategoryFilter.value;

          let filteredItems = filteredPaidData.all;
      ...
      techFilter.addEventListener('change', () => {
       ...
       categoryFilter.addEventListener('change', () => {
           console.log("category filter changed for free starters");
           currentPage = 1;
            filterAndPaginate();
         })
      ...
      paidCategoryFilter.addEventListener('change', () => {
           console.log("Category Filter changed for paid starters.");
           paidCurrentPage = 1;
         filterAndPaginatePaid();
        })
